...
{
    protected BaseRepo(AwesomePizzasContext context)
    {
        _context = context;
    }

    protected readonly AwesomePizzasContext _context;
    ...
    /// <exception cref="EntityNotFoundException">Thrown if the entity with the specified identifier is not found.</exception>
    public async Task<TBaseEntity> GetById(int id)
    {
        lock (_lock)
        {
            ...
            if (entity == null)
                throw new EntityNotFoundException($"{typeof(TBaseEntity)} with key {id} not found!!!");
            return entity;
        }
    }
